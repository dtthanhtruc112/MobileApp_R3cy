...
import static com.example.databases.R3cyDB.*;
...
//    }
    private void btnSaveAddressClicked() {
        ...
        if (receiverName.isEmpty() || receiverPhone.isEmpty() || province.isEmpty() || district.isEmpty() || ward.isEmpty() || detailAddress.isEmpty()) {
            ...
            // Truy vấn SQL để kiểm tra xem địa chỉ mới có giống với bất kỳ địa chỉ nào đã được lưu trước đó của cùng một customerId không
            String query = "SELECT * FROM " + TBl_ADDRESS + " WHERE " + CUSTOMER_ID + " = " + customerId;
            Cursor cursor = db.getData(query);

            // Biến để kiểm tra xem có ít nhất một bản ghi nào đó không giống với dữ liệu mới không
            boolean isDifferent = true;

            // Kiểm tra Cursor và số lượng bản ghi trước khi lặp
            if (cursor != null && cursor.getCount() > 0) {
                // Lặp qua các bản ghi để kiểm tra xem dữ liệu mới có giống với bất kỳ bản ghi nào không
                while (cursor.moveToNext()) {
                    @SuppressLint("Range") String savedReceiverName = cursor.getString(cursor.getColumnIndex(RECEIVER_NAME));
                    @SuppressLint("Range") String savedReceiverPhone = cursor.getString(cursor.getColumnIndex(RECEIVER_PHONE));
                    @SuppressLint("Range") String savedProvince = cursor.getString(cursor.getColumnIndex(PROVINCE));
                    @SuppressLint("Range") String savedDistrict = cursor.getString(cursor.getColumnIndex(DISTRICT));
                    @SuppressLint("Range") String savedWard = cursor.getString(cursor.getColumnIndex(WARD));
                    @SuppressLint("Range") String savedDetailAddress = cursor.getString(cursor.getColumnIndex(ADDRESS_DETAILS));
                    @SuppressLint("Range") int savedIsDefault = cursor.getInt(cursor.getColumnIndex(IS_DEFAULT));
                    @SuppressLint("Range") String savedAddressType = cursor.getString(cursor.getColumnIndex(ADDRESS_TYPE));
            ...
            // Nếu dữ liệu mới khác với dữ liệu đã lưu
            if (isDifferent) {
                // Thực hiện lưu địa chỉ xuống cơ sở dữ liệu
                String insertQuery = "INSERT INTO " + TBl_ADDRESS + "(" +
                        CUSTOMER_ID + ", " +
                        RECEIVER_NAME + ", " +
                        RECEIVER_PHONE + ", " +
                        PROVINCE + ", " +
    ...
    private void setDefaultAddress(int customerId, int newAddressId) {
        ...
        String updateQuery = "UPDATE " + TBl_ADDRESS +
                " SET " + IS_DEFAULT + " = 0" +
